...
interface QuotationDetailSheetProps {
  ...
  onSendToPos: (id: string) => Promise<void>
...
export function QuotationDetailSheet({
  ...
  const canSendToPos = status === 'sent'
  const canReject = status === 'draft' || status === 'sent' || status === 'accepted'
  const hasActions = canMarkSent || canSendToPos || canReject
              ...
              <table className="w-full border-collapse text-xs">
                ...
                <thead>
                  <tr className="border-b border-border bg-muted/40 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
                    ...
                    <th className="pr-3 py-1.5 text-right font-semibold">Price</th>
                    <th className="py-1.5 text-center font-semibold">Tax%</th>
                    <th className="pr-3 py-1.5 text-right font-semibold">Amount</th>
                    <th className="py-1.5 text-center font-semibold">Qty</th>
                    <th className="pr-3 py-1.5 text-right font-semibold">Total</th>
                  </tr>
                </thead>
                  ...
                  {quotation.quotation_items.map((item) => {
                    ...
                    return (
                      <tr key={item.id}>
                        ...
                        <td className={cn(
                          'pr-3 py-2 text-right tabular-nums',
                          addTaxPct > 0 ? 'text-blue-600 dark:text-blue-400' : 'text-muted-foreground'
                        )}>
                        ...
                        <td className="pr-3 py-2 text-right font-semibold tabular-nums text-foreground">
              ...
              <div className="flex gap-2">
                ...
                {canSendToPos && (
                  <Button
                    size="sm"
                    disabled={isPending}
                    onClick={() => runAction(() => onSendToPos(quotation.id))}
                  >
